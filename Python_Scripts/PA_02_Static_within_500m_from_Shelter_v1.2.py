...
Purpose:
This script creates "Static Within 500m from Shelter" raster.

Usage:
Static_within_500m_from_Shelter.py ifc a6r [-h] [-l] [-ld]

Positional Arguments:
   fdp              Feature Class containing "W_Shelter_1" HEM field
   ifc              Feature Class containing "Static_WFD_All" HEM field (same path as above usually)
...
Example Input:
X:\fullpath\Static_within_500m_from_Shelter.py Y:\fullpath\ifc Z:\fullpath\a6r

History
2020-02-18 (JK): Created script Static_within_500m_from_Shelter.
"""
...
def main(input_fc, depth_fc, aoi6_r):
    ...
    for fc in [input_fc, depth_fc, aoi6_r]:
        if not arcpy.Exists(fc):
            logging.error("**** Specified input " + fc + " does not exist. Exiting script.")
            sys.exit()
    exit_script = False
    for required_field in ["W_Shelter_1"]:
        if required_field not in [f.name for f in arcpy.ListFields(depth_fc)]:
            logging.error("**** Specified 400m depth polygon feature class does not contain required field " + \
                          required_field + ". Exiting script.")
            exit_script = True
    if exit_script:
        sys.exit()
    exit_script = False
    for required_field in ["Static_WFD_All"]:
    ...
    # ---------------------------------------------------------------------------------------------------------
    # Buffer the polygons with Shelter_1 = 1 by 100m and convert to raster
    # ---------------------------------------------------------------------------------------------------------

    security_fl = "shelter_fl"
    where_clause = '"W_Shelter_1" = 1'
    logging.info("Selecting polygons where W_Shelter_1 = 1")
    arcpy.MakeFeatureLayer_management(depth_fc, security_fl, where_clause)
    
    buff_fc = os.path.join(scratch_fgdb, "Buffer_to_Select_Static_Forage_500m")
    if arcpy.Exists(buff_fc):
        arcpy.Delete_management(buff_fc)
    logging.info("Buffering by 500m")
    arcpy.Buffer_analysis(security_fl, buff_fc, "500 Meters", "OUTSIDE_ONLY")
    ...
    # ---------------------------------------------------------------------------------------------------------
    # Select the shelter polygons with Static_WFD_All = 1 and convert to raster
    # ---------------------------------------------------------------------------------------------------------

    static_forage_fl = "static_forage_fl"
    where_clause = '"Static_WFD_All" = 1'
    logging.info("Selecting polygons where Static_WFD_All = 1")
    arcpy.MakeFeatureLayer_management(input_fc, static_forage_fl, where_clause)
    
    static_forage_r = os.path.join(scratch_fgdb, "Static_All_meets_site_cond")
    if arcpy.Exists(static_forage_r):
        arcpy.Delete_management(static_forage_r)
    logging.info("Converting selected polygons to raster: " + static_forage_r)
    arcpy.PolygonToRaster_conversion(static_forage_fl, "Static_WFD_All", static_forage_r, "CELL_CENTER", "#", 100)
    ...
    merged_r_name = "Static_WFD_All_500m_Shelter1_temp"
    ...
    # arcpy.MosaicToNewRaster_management([buff_r, static_forage_r, aoi6_r], scratch_fgdb, merged_r_name, "#", "#", "#",
    #                                    1, "SUM")
    logging.info("Calculating weighted sum of the two new rasters with the AOI_WMUs_6 raster")
    arcpy.gp.WeightedSum_sa(buff_r + " Value 1;" + static_forage_r + " Value 1;" + aoi6_r + " Value 1", merged_r)

    # ---------------------------------------------------------------------------------------------------------
    # Reclassify raster so cells with value 11 are assigned a 1 and all others assigned a 0
    # ---------------------------------------------------------------------------------------------------------

    merged_reclass_r = os.path.join(os.path.split(input_fc)[0], "Static_WFD_All_500m_Shelter1")
...
if __name__ == '__main__':
    try:
        ...
        parser.add_argument('ifc', help='Shelter polygons')
        parser.add_argument('fdp', help='400m Depth polygons')
        ...
        # Start the script
        main(args.ifc, args.fdp, args.a6r)
