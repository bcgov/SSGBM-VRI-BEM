create_grid_from_iteration_number <- function(n_iterations, wkt_filter, as.text = TRUE) {
  ...
  if ((sqrt_n_iterations %% 1) == 0) {
    grid <-  st_make_grid(st_as_sfc(st_as_text(wkt_filter)), n = c(sqrt_n_iterations,sqrt_n_iterations))
  }
  else {
    base <- ceiling(sqrt_n_iterations)
    grid <- st_make_grid(st_as_sfc(st_as_text(wkt_filter)), n = c(base, base/n_iterations))
  }
