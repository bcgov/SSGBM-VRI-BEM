...
calc_capability_rating <- function(rrm_dt, animal) {
  ...
  if (animal == "moose") {
    rrm_dt[ , (cap_rating_variables) := lapply(.SD, min), by = .(Eco_sec, Bgc_zone, Bgc_subzon, Bgc_vrt, Bgc_phase, Beumc, Slope_mod, Site_m3a,
                                                                 Snow_code, Above_Elev_Thold, Crown_Moose), .SDcols = (rating_variables)]
